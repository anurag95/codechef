...
int main()
{
	...
	if(n == 0)
		...
		double dist;
		int min = 2;
		pair<double, double> midpt, p1, p2;
		double slope;
		sort(pt, pt+n);
		bool a, b;
		int anurag=0;
		...
		for(i=0;i<n-1;i++)
		{
			for(j=i+1;j<n;j++)
			{
				...
				if(anurag == 0)
				{
					p1.x = midpt.x + (dist/2)/sqrt(1 + slope*slope);
					p2.x = midpt.x - (dist/2)/sqrt(1 + slope*slope);	
					p1.y = midpt.y + (dist/2)*slope/sqrt(1 + slope*slope);
					p2.y = midpt.y - (dist/2)*slope/sqrt(1 + slope*slope);
				}
				else {
					p1.x = p2.x = midpt.x;
					p1.y = midpt.y + (dist/2);
